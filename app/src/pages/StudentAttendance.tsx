...
import { Users, User, Search, ChevronLeft, Calendar, CheckCircle, XCircle, ChevronRight, X, MessageSquare, Plus, Camera } from 'lucide-react';
    ...
    return (
        <div className="p-3 sm:p-8 max-w-[1600px] mx-auto space-y-6 sm:space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col gap-4 mb-2">
                ...
                >
                    <div className="flex items-center gap-3 w-full sm:w-auto group">
                        <div className="w-10 h-10 flex items-center justify-center bg-primary/5 rounded-full border border-primary/10 transition-all duration-500 group-focus-within:bg-primary/20">
                        ...
                        <input
                            ...
                            className="bg-transparent backdrop-blur-md border-b border-white/10 text-white text-base font-black rounded-none focus:ring-0 focus:border-primary block w-full sm:w-72 px-4 py-3 transition-all text-center tracking-[0.2em] uppercase"
                        />
                         <button
                            onClick={() => setShowQRModal(true)}
                            className="p-3 rounded-full bg-primary/10 border border-primary/20 text-primary hover:bg-primary hover:text-white transition-all shadow-lg"
                            title="QR Scan"
                        >
            ...
            <QRScannerModal
                ...
                onScan={(scannedId) => {
                    // Extract ID if it's a URL or complex string
                    const studentId = scannedId.includes('/') ? scannedId.split('/').pop() : scannedId;
                    if (studentId) {
                        handleStatusUpdate(studentId, 'present');
                    }
                }}
            />
        </div>
            ...
            <div className="glass-card w-full max-w-lg rounded-2xl border border-white/10 overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
                <div className="p-5 border-b border-white/10 flex items-center justify-between bg-white/5">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="p-1.5 bg-white rounded-lg hidden sm:block">
                            <QRCodeSVG value={studentId} size={40} />
                        </div>
