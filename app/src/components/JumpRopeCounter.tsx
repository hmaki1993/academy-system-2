...
const JumpRopeCounter = () => {
    ...
    const handleCameraError = (err: any) => {
        ...
        const msg = (err?.name === 'NotFoundError' || err?.message?.includes('Requested device not found'))
            ? "No camera found. Please connect a webcam to count jumps."
            ...
            if (isSessionRunning) {
                ...
                if (timerRemainingRef.current !== null) {
                    ...
                    if (nextValue === 0) {
                        ...
                        setShowSummary(true);
                        speak(`Time's up! You did ${jumpCountRef.current} jumps. Great work!`);
                    }
                } else {
                ...
                <div className="w-full mt-4 glass-panel p-4 animate-in fade-in slide-in-from-top-4 duration-500">
                    ...
                    {history.length > 1 && !historyLoading && (
                        <div className="mt-6 pt-6 border-t border-white/5 overflow-hidden">
                            <h4 className="text-[8px] font-black text-white/30 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                                <TrendingUp className="w-3 h-3 text-cyan-400" />
                                JPM Intensity Trends
                            </h4>
                            <div className="h-32 w-full">
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={[...history].reverse()}>
                                        <defs>
                                            <linearGradient id="colorJpm" x1="0" y1="0" x2="0" y2="1">
                                        ...
                                        <Tooltip
                                            ...
                                            labelFormatter={() => "Intensity Trend"}
                                        />
                                        <Area
                                            type="monotone"
                                            dataKey="jpm"
