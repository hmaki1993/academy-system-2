...
export default function Students() {
    ...
    return (
        <div className="space-y-6 sm:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            {/* Stats Cards - Elite Style */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    { label: t('dashboard.totalStudents'), value: students.length, icon: Users, color: 'primary' },
                    { label: t('students.active'), value: students.filter(s => s.is_active).length, icon: Check, color: 'emerald' },
                    { label: t('students.inactive'), value: students.filter(s => !s.is_active).length, icon: X, color: 'rose' }
                ].map((stat, i) => (
                    <div key={i} className="glass-card p-6 group relative overflow-hidden">
                        <div className={`absolute -top-10 -right-10 w-24 h-24 rounded-full blur-3xl opacity-5 group-hover:opacity-100 transition-opacity bg-${stat.color}-500/10`}></div>
                        <div className="relative z-10 flex items-center justify-between">
                            ...
                            <div className={`w-12 h-12 rounded-xl bg-${stat.color}-500/10 flex items-center justify-center text-${stat.color}-400 group-hover:scale-105 transition-all duration-300`}>
            ...
            <div className="glass-card p-6 md:p-8 rounded-3xl border border-white/5 relative overflow-hidden bg-gradient-to-br from-white/[0.02] to-transparent">
                ...
                <div className="relative z-10">
                    <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6">
                        <div className="flex items-center gap-4">
                            <div className="w-14 h-14 bg-gradient-to-br from-primary to-accent rounded-2xl shadow-lg flex items-center justify-center">
                                <TrendingUp className="w-7 h-7 text-white" />
                            </div>
                            <div>
                                <h2 className="text-2xl font-black text-white uppercase tracking-tight leading-none">{t('pt.title')}</h2>
                                <p className="text-[10px] font-bold text-white/30 uppercase tracking-widest mt-1">{t('pt.subtitle')}</p>
                            </div>
                        </div>
                        {role === 'admin' && (
                            <button
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {ptSubscriptions.map((subscription) => (
                                <div
                                    ...
                                    className="glass-card p-4 rounded-2xl border-white/5 hover:border-white/20 transition-all duration-300 group flex flex-col h-full bg-[#0a0c10]/40 cursor-pointer"
                                >
            ...
            <div className="glass-card rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden border border-white/10 shadow-premium">
                <div className="flex flex-col md:flex-row p-4 sm:p-6 md:p-8 border-b border-white/5 gap-4 sm:gap-6 bg-white/[0.02] items-center">
                    ...
                    <div className="w-full md:flex-1 md:max-w-md flex items-center gap-4 group order-2 md:order-none relative">
                        <Search className={`w-5 h-5 ${searchTerm ? 'text-primary animate-pulse' : 'text-white/20'} transition-colors duration-300 shrink-0`} />
