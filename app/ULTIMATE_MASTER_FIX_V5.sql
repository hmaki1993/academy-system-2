-- ============================================================
-- 🏆 ULTIMATE MASTER FIX (V5 - FULL PROJECT SYNCHRONIZATION)
-- ============================================================
-- Project: Epic Academy System (System 2)
-- Goal: 100% Core Alignment with System 1 + Bug Fixes
-- Includes: Finance, PT, Attendance, Chat, and Storage
-- ============================================================
...
-- Gym Settings (Fixes 406 Error - Ensures only 1 row)
...
-- Delete duplicates from gym_settings
...
-- [4] Finance Module (Sync from System 1)
...
-- [5] PT Module (Sync from System 1 + Optimized)
...
-- [9] RPC Functions (Parameter & Logic Fixes)

-- Fixed: target_user_id (to match Coaches.tsx)
...
-- Standard: create_new_user (Full Metadata & Identities Fix)
...
DECLARE
    ...
    -- Ensure email_verified is true in metadata
    full_metadata JSONB := user_metadata || '{"email_verified": true}'::jsonb;
BEGIN
    -- 1. Insert the auth user
...
-- Standard: handle_new_user (Dynamic Role Fix)
...
BEGIN
  ...
  VALUES (
    ...
    COALESCE(NEW.raw_user_meta_data->>'role', 'coach') -- Dynamic Role (Respects Auth Metadata)
  )
...
-- [11] Retroactive Fix: Repair Any Existing Users Missing Identities (Fixes 500 Login Error)
INSERT INTO auth.identities (
...
-- [12] Retroactive Fix: Repair NULL Tokens in auth.users
UPDATE auth.users 
SET 
...
-- [13] Fix Login Page: Allow anon users to READ gym_settings
-- (Required so the Login page can load the custom theme before the user is authenticated)
DROP POLICY IF EXISTS "Auth access" ON public.gym_settings;
...
-- Allow ANYONE (including visitors on the login page) to read settings
CREATE POLICY "Public Read gym_settings" ON public.gym_settings
  FOR SELECT USING (true);

-- Allow only authenticated users (admins/coaches) to INSERT settings
CREATE POLICY "Auth Insert gym_settings" ON public.gym_settings
  FOR INSERT TO authenticated WITH CHECK (true);

-- Allow only authenticated users to UPDATE settings
CREATE POLICY "Auth Update gym_settings" ON public.gym_settings
  FOR UPDATE TO authenticated USING (true) WITH CHECK (true);

-- [14] Ensure gym_settings has at least 1 row so the app can always save
INSERT INTO public.gym_settings (academy_name)
